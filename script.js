...
function openProfile(nick) {
    ...
    // Запрос полноценного 3D-скина во весь рост по нику
    skinImg.src = `https://surgeplay.com{player.nick}`;

    // Защита: если у игрока нет лицензии, отобразится дефолтный Стив
    skinImg.onerror = function() {
        skinImg.onerror = null;
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="player-cell" onclick="openProfile('${player.nick}')"> 
                        <!-- Запрос 3D головы 32х32 пикселя с защитой на случай пиратки -->
